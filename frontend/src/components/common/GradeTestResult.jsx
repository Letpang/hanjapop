    ...
    return (
        <div className={`w-full min-h-[100dvh] flex flex-col items-center justify-center px-5 py-4 overflow-hidden ${passed ? 'bg-[#E8F8F3]' : 'bg-[#FFF1F1]'}`}>
            <div className="w-full max-w-md rounded-[2.5rem] bg-white px-6 py-6 shadow-2xl shadow-[rgba(91,103,122,0.14)] flex flex-col items-center text-center">
                ...
                {!passed && (
                    <p className="text-body-sm font-black text-[#AEB7C5] mb-1.5">
                        괜찮아요, 다시 도전해봐요!
                    </p>
                )}
                <h3 className={`text-[2rem] leading-tight font-black tracking-tight break-keep ${passed ? 'text-[#FF8F6B]' : 'text-[#FF6B6B]'}`}>
                    {title}
                </h3>
                <p className="text-body-sm font-extrabold text-[#AEB7C5] mt-2 break-keep">
                    {subtitle}
                </p>

                <div className="w-full mt-5 rounded-[1.75rem] border-4 border-[#E9EDF2] bg-white px-5 py-4 shadow-inner">
                    <div className="flex items-end justify-center gap-2">
                        <span className={`text-[3rem] leading-none font-black tracking-tight ${passed ? 'text-[#00A891]' : 'text-[#FF6B6B]'}`}>
                            {correct}
                        </span>
                        <span className="text-[2.35rem] leading-none font-black text-[#334155]">/</span>
                        <span className="text-[3rem] leading-none font-black tracking-tight text-[#334155]">
                            {total}
                        </span>
                    </div>
                    <p className="text-body-sm font-black text-[#AEB7C5] mt-2">
                ...
                {passed && !alreadyUnlocked && (
                    <div className="w-full mt-4 rounded-[1.75rem] bg-[#EFFFFB] border-2 border-[#8FEBDD] px-4 py-3.5 flex items-center justify-center gap-3 shadow-sm">
                        ...
                        <p className="text-[1.35rem] leading-snug font-black text-[#007C6D] break-keep">
                            {unlockText}
                        </p>
                    </div>
                )}
