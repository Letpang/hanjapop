        ...
        {GRADES.map(g => {
            ...
            return (
                <button
                    ...
                    className={`relative h-[5.2rem] rounded-3xl font-bold text-h3 transition-all border-4 flex flex-col items-center justify-center gap-1 shadow-sm active:scale-95 ${
                        locked 
...
export const TopicCard = ({ name, isSelected, onClick, locked }) => {
    ...
    <button
        onClick={handleClick}
        className={`relative h-[5.2rem] rounded-3xl font-bold text-h3 transition-all border-4 flex items-center justify-center shadow-sm active:scale-95 px-2 ${
            locked
            ? 'bg-[#F8FAF9] border-[#E9EDF2] text-slate-200 cursor-not-allowed'
            : isSelected
                ? 'bg-white border-[#FFA88D] text-[#5B677A] shadow-lg'
