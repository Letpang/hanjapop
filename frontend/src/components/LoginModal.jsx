...
export default function LoginModal({ onClose, platform, signInWithApple, signInWithGoogle, signInWithKakao }) {
    ...
    const handleKakao = async () => {
        ...
        setLoading(false);
        if (result?.success) onClose?.();
        else setError('카카오 로그인에 실패했습니다. 다시 시도해주세요.');
    };
            ...
            <div className="login-modal-card w-full max-w-md rounded-t-3xl p-8 pb-12"
                ...
                {platform === 'ios' && (
                    <button
                        onClick={handleApple}
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl mb-3 font-normal text-white"
                ...
                {platform === 'android' && (
                    <button
                        ...
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl mb-3 font-normal border text-[#334155]"
                ...
                <button
                    onClick={handleKakao}
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl mb-3 font-normal text-[#191919]"
                    style={{ background: '#FEE500', opacity: loading ? 0.6 : 1 }}>
