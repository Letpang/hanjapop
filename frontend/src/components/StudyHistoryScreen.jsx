  ...
  return (
    <section className={`rounded-[2rem] border p-5 shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-white'}`}>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <p className="text-[11px] font-black tracking-wider text-[#8D9CAE]">최근 14일</p>
          <h3 className={`text-lg font-black ${isDarkMode ? 'text-white' : 'text-[#334155]'}`}>학습 그래프</h3>
        </div>
        <div className="rounded-2xl bg-[#F8FAF9] px-3 py-2 text-right">
          <p className="text-[10px] font-black text-[#8D9CAE]">총 학습 문제</p>
          <p className="text-base font-black text-[#00A994]">{totals.questions}문제</p>
        </div>
      </div>
      ...
      <div className="mb-4 grid grid-cols-4 gap-1.5 rounded-2xl bg-[#F4F7F8] p-1.5">
        {graphTabs.map(({ id, label }) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`h-9 rounded-xl text-xs font-black transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00C7AE]/30 ${tab === id ? 'bg-white text-[#00A994] shadow-sm' : 'text-[#8D9CAE]'}`}
          >
            {label}
          </button>
        ))}
      ...
      {tab === 'xp' && (
        <>
          ...
          <div className="mt-3 rounded-xl bg-[#FFF1EA] px-3 py-2 text-xs font-black text-[#E8664F]">
            최근 14일 {totals.xp} XP
          </div>
        </>
      )}
      ...
      {tab === 'wrong' && (
        <>
          ...
          <div className="mt-3 rounded-xl bg-[#FFF1EE] px-3 py-2 text-xs font-black text-[#C94C3B]">
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-lg flex-col gap-5 px-5 pt-4 safe-top safe-bottom">
        <header className={`flex min-h-[64px] items-center justify-between rounded-[2rem] border px-4 shadow-sm ${isDarkMode ? 'bg-slate-800/90 border-slate-700' : 'bg-white/90 border-white'}`}>
          ...
          <div className="text-center">
            <h2 className={`text-lg font-black ${isDarkMode ? 'text-white' : 'text-slate-700'}`}>날짜별 현황</h2>
        ...
        <section className={`rounded-[2rem] border p-5 shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-white'}`}>
          <div className="mb-4 flex items-center justify-between">
            <button
              ...
              className={`h-10 w-10 rounded-2xl border font-black ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-[#F4F6F8] border-slate-100'}`}
            >
              ‹
            </button>
            <div className="text-center">
              <p className="text-[11px] font-black tracking-[0.2em] text-[#8D9CAE]">{year}</p>
              <p className="text-xl font-black text-[#00A994]">{month + 1}월</p>
            </div>
            <button
              onClick={() => setViewDate(new Date(year, month + 1, 1))}
              className={`h-10 w-10 rounded-2xl border font-black ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-[#F4F6F8] border-slate-100'}`}
          ...
          <div className="mb-2 grid grid-cols-7 text-center text-xs font-black text-[#8D9CAE]">
          ...
          <div className="grid grid-cols-7 gap-2">
            {cells.map((day, idx) => {
              ...
              const base = 'aspect-square rounded-2xl text-xs font-black transition-all active:scale-95';
              const classes = {
          ...
          <div className="mt-4 flex items-center justify-center gap-4 rounded-2xl bg-[#F8FAF9] py-2 text-[11px] font-black text-[#5D677A]">
        ...
        <section className={`rounded-[2rem] border p-5 shadow-sm ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-white'}`}>
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-black tracking-wider text-[#8D9CAE]">{selectedDay.replace(/-/g, '.')}</p>
              <h3 className={`text-lg font-black ${isDarkMode ? 'text-white' : 'text-[#334155]'}`}>학습 내용</h3>
            </div>
            <span className="rounded-full bg-[#F4F6F8] px-3 py-1 text-xs font-black text-[#5D677A]">
                ...
                <DetailSection title="완료한 활동" count={selectedSummary.activities.length} tone="teal">
                  <div className="grid grid-cols-2 gap-2">
                    {selectedSummary.activities.map(type => (
                      <div key={type} className="rounded-2xl bg-[#F8FAF9] px-3 py-2 text-sm font-extrabold text-[#45546A]">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedSummary.hanjas.map(h => (
                      <div key={h.id} className="rounded-2xl border border-slate-100 bg-white px-3 py-2 shadow-sm">
                        <span className="mr-2 text-lg font-black text-[#334155]">{h.hanja}</span>
                        <span className="text-xs font-black text-[#94A3B8]">{h.sound} · {h.meaning}</span>
                      </div>
                    ))}
                  </div>
                </DetailSection>
                  ...
                  <div className="flex flex-col gap-2">
                    {selectedSummary.correctWords.map(w => (
                      <div key={w.id} className="flex items-center justify-between rounded-2xl bg-[#F8FAF9] px-3 py-2">
                        <span className="font-black text-[#334155]">{w.word}</span>
                        <span className="text-xs font-extrabold text-[#94A3B8]">{w.reading}</span>
                      </div>
                    ))}
                  </div>
                </DetailSection>
                  ...
                  <div className="flex flex-col gap-2">
                    {selectedSummary.wrongWords.map(w => (
                      <div key={w.id} className="flex items-center justify-between rounded-2xl bg-[#FFF1EE] px-3 py-2">
                        <span className="font-black text-[#C94C3B]">{w.word}</span>
                        <span className="text-xs font-extrabold text-[#E8664F]">{w.reading}</span>
                      </div>
                    ))}
                  </div>
                </DetailSection>
