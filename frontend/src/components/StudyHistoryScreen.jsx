...
const StudyGraph = ({ days, isDarkMode }) => {
  ...
  return (
    <section className={`rounded-[2rem] border p-5 shadow-sm bg-white dark:bg-slate-800 border-white dark:border-slate-700 dark:bg-slate-800 dark:border-slate-700`}>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <p className="text-[11px] font-normal tracking-wider text-[#8D9CAE]">최근 14일</p>
          <h3 className={`text-lg font-medium text-[#334155] dark:text-white`}>학습 그래프</h3>
        </div>
        <div className="rounded-2xl bg-[#F8FAF9] dark:bg-slate-900 px-3 py-2 text-right">
      ...
      <div className="mb-4 grid grid-cols-4 gap-1.5 rounded-2xl bg-[#F4F7F8] dark:bg-slate-900 p-1.5">
        {graphTabs.map(({ id, label }) => (
          <button
            key={id}
            onClick={() => setTab(id)}
            className={`h-9 rounded-xl text-xs font-normal transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#00C7AE]/30 ${tab === id ? 'bg-white dark:bg-slate-800 text-[#00A994] shadow-sm' : 'text-[#8D9CAE]'}`}
          >
            {label}
          </button>
        ))}
      ...
      {tab === 'questions' && (
        <>
          <div className="rounded-[1.5rem] bg-[#FBFCFD] dark:bg-slate-900 px-3 py-4">
            <div className="grid h-36 items-end gap-1" style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))` }}>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 text-[11px] font-normal text-center">
            <span className="rounded-xl bg-[#E8FAF7] dark:bg-teal-950/40 px-2 py-2 text-[#00A994] dark:text-teal-300">학습지 {items.reduce((sum, item) => sum + item.flashcard, 0)}</span>
            <span className="rounded-xl bg-[#F0EFFF] dark:bg-indigo-950/40 px-2 py-2 text-[#6F63E8] dark:text-indigo-300">단어 {items.reduce((sum, item) => sum + item.wordQuiz, 0)}</span>
            <span className="rounded-xl bg-[#FFF1EA] dark:bg-rose-950/40 px-2 py-2 text-[#E8664F] dark:text-rose-300">문장 {items.reduce((sum, item) => sum + item.sentenceQuiz, 0)}</span>
          </div>
        </>
      )}
      ...
      {tab === 'xp' && (
        <>
          ...
          <div className="mt-3 rounded-xl bg-[#FFF1EA] dark:bg-rose-950/40 px-3 py-2 text-xs font-normal text-[#E8664F] dark:text-rose-300">
            최근 14일 {totals.xp} XP
          </div>
        </>
      )}
      ...
      {tab === 'words' && (
        <>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] font-normal text-center">
            <span className="rounded-xl bg-[#E8FAF7] dark:bg-teal-950/40 px-2 py-2 text-[#00A994] dark:text-teal-300">한자 {totals.hanja}</span>
            <span className="rounded-xl bg-[#F0EFFF] dark:bg-indigo-950/40 px-2 py-2 text-[#6F63E8] dark:text-indigo-300">단어 {totals.words}</span>
          </div>
        </>
      )}
      ...
      {tab === 'wrong' && (
        <>
          ...
          <div className="mt-3 rounded-xl bg-[#FFF1EE] dark:bg-rose-950/40 px-3 py-2 text-xs font-normal text-[#C94C3B] dark:text-rose-300">
...
const StudyHistoryScreen = ({ onBack, isDarkMode }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 overflow-y-auto bg-[#F7FAF9] dark:bg-slate-900 text-[#334155] dark:text-slate-200 dark:bg-slate-900 dark:text-white`}>
      <div className="mx-auto flex min-h-screen w-full max-w-lg flex-col gap-5 px-5 pt-4 safe-top safe-bottom">
        <header className={`flex min-h-[64px] items-center justify-between rounded-[2rem] border px-4 shadow-sm bg-white dark:bg-slate-800/90 border-white dark:border-slate-700 dark:bg-slate-800/90 dark:border-slate-700`}>
          <button
            onClick={onBack}
            className={`h-10 w-10 rounded-2xl border font-normal shadow-sm active:scale-95 bg-white dark:bg-slate-800 border-slate-100 text-[#5B677A] dark:text-slate-300 dark:bg-slate-700 dark:border-slate-600 dark:text-white`}
          >
            ←
          </button>
          <div className="text-center">
            <h2 className={`text-lg font-medium text-slate-700 dark:text-white`}>날짜별 현황</h2>
        ...
        <section className={`rounded-[2rem] border p-5 shadow-sm bg-white dark:bg-slate-800 border-white dark:border-slate-700 dark:bg-slate-800 dark:border-slate-700`}>
          ...
          <div className="grid grid-cols-7 gap-2">
            {cells.map((day, idx) => {
              ...
              const classes = {
                ...
                done: 'bg-white dark:bg-slate-800 text-[#334155] dark:text-slate-200 border-2 border-[#00C7AE]/45',
          ...
          <div className="mt-4 flex items-center justify-center gap-4 rounded-2xl bg-[#F8FAF9] dark:bg-slate-900 py-2 text-[11px] font-normal text-[#5D677A]">
        ...
        <section className={`rounded-[2rem] border p-5 shadow-sm bg-white dark:bg-slate-800 border-white dark:border-slate-700 dark:bg-slate-800 dark:border-slate-700`}>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <h3 className={`text-lg font-medium text-[#334155] dark:text-white`}>학습 내용</h3>
            </div>
                ...
                <DetailSection title="완료한 활동" count={selectedSummary.activities.length} tone="teal">
                  <div className="grid grid-cols-2 gap-2">
                    {selectedSummary.activities.map(type => (
                      <div key={type} className="rounded-2xl bg-[#F8FAF9] dark:bg-slate-900 px-3 py-2 text-sm font-normal text-[#45546A] dark:text-slate-300">
