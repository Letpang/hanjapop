...
const ShootGameScreen = ({ onBack, onGameFinish, onHanjaAcquired, selectedCharacter, getRewardPreview, onMarkWrong, onMarkCorrect, onWordCorrect, onWordWrong, onWaveClear, masteryData, srsData, userLevel, contentPool, unlockedHanjaIds, currentDayHanjaIds, currentDay, seenHanjaIds, onHanjaSeen, onWordSeen, dailyMapNode, hideRetry, autoStart, missionDone = false, killsPerWaveOverride, userXpOverride }) => {
    ...
    // 낙하 + 스폰 루프
    useEffect(() => {
        if (status !== 'playing' || waveTransition || isPaused) return;
    ...
    }, [status, wave, waveTransition, isPaused, gamePoolData, masteryData, getDropSpeed, getSpawnInterval, getMeaning, diffConfig, refillWordQueue]);
            ...
            <div className={`w-full mx-auto h-full flex flex-col relative ${shake ? "animate-shake" : ""}`}>
                ...
                <div className="absolute top-0 left-2 right-2 z-40 safe-top pt-3 flex items-center gap-1.5">
                    ...
                    <div className="h-9 bg-white dark:bg-slate-800/90 backdrop-blur-md rounded-2xl px-3 shadow-md border border-white dark:border-slate-700/50 flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <span className="text-xs font-normal text-[#AEB7C5] tabular-nums whitespace-nowrap">{waveKills}/{diffConfig.killsPerWave}</span>
                    </div>
                ...
                <div className="flex-1 relative overflow-hidden min-h-0" ref={gameAreaRef}>
                    ...
                    {/* 캐릭터 */}
                    {(() => {
                        const charSize = { garae: 'w-20 h-20', jeolmi: 'w-24 h-24', chapssal: 'w-24 h-24', muzi: 'w-20 h-20' };
                        const sz = charSize[selectedCharacter] || 'w-20 h-20';
                        return (
                            <div className="absolute bottom-[4%] left-1/2 -translate-x-1/2 z-30" ref={shipRef}>
                                <div className={`${sz} transition-transform duration-100 drop-shadow-2xl`} style={{ transform: 'rotate(' + (turretAngle + 90) + 'deg)' }}>
                    ...
                    {/* 획득 애니메이션 */}
                    {acquisitions.map(a => (
                        <div key={a.id} className="absolute pointer-events-none z-30 animate-float font-normal text-[#FFB433] text-2xl drop-shadow-lg" style={{ left: a.x + '%', top: a.y + '%', transform: 'translate(-50%, -50%)' }}>
                            +3 XP ✨
                        </div>
                    ))}
                </div>
                    ...
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center" style={{ background: 'rgba(10,15,30,0.72)', backdropFilter: 'blur(8px)' }}>
                        <div className="bg-white dark:bg-slate-800/10 border border-white dark:border-slate-700/20 rounded-3xl px-8 py-10 flex flex-col items-center gap-6 shadow-2xl max-w-xs w-full mx-4">
                            <div className="flex flex-col items-center gap-1">
                                ...
                                <p className="text-white/60 text-sm text-center break-keep">게임이 멈췄어요. 준비되면 계속해요!</p>
                            </div>
                            <button
