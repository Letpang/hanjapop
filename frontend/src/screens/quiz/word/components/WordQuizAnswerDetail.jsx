...
import { getWordTermSizeClass } from '../wordQuizUtils.js';
...
const WordQuizAnswerDetail = ({ isSpeaking, onSpeak, q }) => {
  const charChips = getUniqueHanjaChips(q.word);
  ...
  return (
    <div className="quiz-card-answer-detail flex flex-col h-full px-3 pt-2 pb-3">
      ...
      <div className="quiz-card-answer-main flex-1 flex flex-col items-center justify-center gap-4">
        <span className={`word-quiz-term hanja-char font-normal text-[#1e293b] dark:text-slate-50 drop-shadow-sm text-center leading-none ${getWordTermSizeClass(q.word)}`}>
          {q.word}
        </span>
        <span className="quiz-card-answer-reading text-[1.45rem] font-normal text-slate-500 dark:text-slate-400 leading-tight">{q.reading}</span>
        {charChips.length > 0 && (
          <div className="quiz-card-hanja-chips flex items-center gap-2.5 flex-wrap justify-center max-w-full">
            {charChips.map(({ char, meaning, sound }) => (
              <div key={char} className="quiz-card-hanja-chip flex items-center gap-1.5 bg-[#EEF0FF] dark:bg-indigo-950/40 rounded-2xl px-3 py-2 shadow-sm">
                ...
                <span className="text-[#8F99AD] leading-none">{meaning}</span>
...
export default WordQuizAnswerDetail;
