...
| 레벨 | 필요 누적 XP | 랭크 칭호 | 캐릭터 이미지 단계 | 도달 예상 |
|-----|------------|---------|-----------------|---------|
| Lv.1 | 0 | 새싹 | rank_1 | — |
| Lv.2 | 300 | 새싹 | rank_1 | ~1일 |
| Lv.3 | 900 | 성장 | rank_1 | ~1일 |
| Lv.4 | 2,100 | 성장 | rank_1 | ~3일 |
| Lv.5 | 4,200 | 중급 | rank_2 ★ | ~7일 |
| Lv.6 | 7,200 | 중급 | rank_2 | ~11일 |
| Lv.7 | 11,400 | 고급 | rank_2 | ~18일 |
| Lv.8 | 16,800 | 고급 | rank_2 | ~26일 |
| Lv.9 | 24,000 | 마스터 | rank_3 ★ | ~37일 |
| Lv.10 | 33,000 | 마스터 | rank_3 | ~51일 |
| Lv.11 | 45,000 | 영웅 | rank_3 | ~69일 |
| Lv.12 | 60,000 | 영웅 | rank_3 | ~92일 |
| Lv.13 | 72,000 | 전설 | rank_4 ★ | ~111일 |
| Lv.14 | 90,000 | 전설 | rank_4 | ~138일 |
| Lv.15 | 112,000 | 신화 | rank_4 | ~172일 |
| Lv.16 | 140,000 | 신화 | rank_4 | ~215일 |
| Lv.17 | 168,000 | 천상 | rank_5 ★ | ~258일 |
| Lv.18 | 205,000 | 천상 | rank_5 | ~315일 |
| Lv.19 | 255,000 | 불멸 | rank_5 | ~392일 |
| Lv.20 | 320,000 | 불멸 | rank_5 (MAX) | ~492일 |

> ★ 캐릭터 이미지 진화 시점. 하루 650 XP 기준 예상치 (스트릭 배율 미적용).
...
활동할 때마다 `onHanjaAcquired`를 통해 즉시 지급. 횟수 제한 없음.

| 활동 | 문제당 XP | 완료 보상 XP |
|------|-----------|-------------|
| 한자 학습지 | 정답당 +5 XP | +50 XP |
| 단어 퀴즈 | 정답당 +5 XP | +20 XP |
| 문장 퀴즈 | 정답당 +10 XP | +20 XP |
| 한자 쓰기 | 1자당 +10 XP | +30 XP (70점 이상) |
| 메모리 게임 | 매칭 1쌍당 +3 XP | +20 XP |
| 몬스터 슈팅 | 처치 1마리당 +3 XP | +20 XP |
| 레벨 테스트 | 정답당 +10 XP | 응시 +20 / 통과 +50 / 만점 +100 XP |
| **학습 지도 완주** | — | **+200 XP** |

### 5-2. 미션 완료 보상 (스테이지당 1회)

`updateMissionProgress` → `stage_missions_${day}` 키로 저장. **커리큘럼 스테이지(단계)당 딱 1회만 지급.**
...
| 단어 퀴즈 1세트 완료 | +30 XP |
| 문장 퀴즈 1세트 완료 | +30 XP |
| 몬스터 슈팅 1웨이브 완료 | +20 XP |
| 메모리 게임 1판 완료 | +20 XP |
| 획순 테스트 1개 완료 | +30 XP |
| **전체 미션 올클리어 보너스** | **+200 XP** |
...
출시 버전에서는 별도 XP 2배 버프를 사용하지 않는다. 모든 XP는 기본 보상 계산 후 스트릭 배율만 적용한다.

```
최종 XP = 기본XP × 스트릭배율
예) 30 XP × 2.0(15일 스트릭) = 60 XP
```
...
| `xp_buff_expires` | 구버전 XP ×2 버프 만료 timestamp. 출시 버전 계산에는 사용하지 않음 |
| `dark_mode` | 다크모드 여부 |
...
| 커리큘럼 | 누적 XP 추정 | 예상 레벨 |
|---------|------------|---------|
| Day 17 완료 (free 구간) | ~3,000~5,000 XP | **Lv.5~6** |
| Day 51 완료 (pack1 구간) | ~20,000~30,000 XP | **Lv.8~9** |
| Day 76 완료 (pack2 중반) | ~50,000~70,000 XP | **Lv.12~13** |
| Day 101 완료 (pack2 후반) | ~80,000~110,000 XP | **Lv.13~14** |
| Day 124 완료 (전체) | ~100,000~150,000 XP | **Lv.14~16** |
| 이후 반복 학습 | 계속 누적 | Lv.16 ~ 20 |

> Lv.17(rank_5 진화, ~258일)은 커리큘럼 완료 후 반복 학습으로 달성.  
